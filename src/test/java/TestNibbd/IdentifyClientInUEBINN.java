...
public class IdentifyClientInUEBINN {
    ...
    private final static String ERRORTEXT = "java.lang.RuntimeException: Client doesn't have attribute [ClientId = 915, AttrRefId = UNN (Идентификационный номер налогоплательщика)]";
    private final static String ERRORTEXTTWO = "Business entity data not found";
    private final static Integer BRANCHID = 1104;
    private final static Integer CLIENTIDOK = 17017;
    private final static Integer CLIENTIDERROR = 915;
    private final static Integer CLIENTIDERRORTWO = 55844;
    private final static Integer HTTPSTATUSOK = 200;
    private final static Integer HTTPSTATUSERROR = 500;

    @Test(groups = {"EBP"})
    @Description("Отправка запроса в ЕБП на идентификацию клиента ЮР по ИНН")
    public void identifyClientInUEBPositive() {
        //Отправка запроса по идентификации ЮЛ в ЕБП
    ...
    @Test(groups = {"EBP"})
    @Description("Отправка запроса в ЕБП на идентификацию клиента ИП по ПИНФЛ - по клиенту отсутсвуют обязательные атрибуты ИНН ")
